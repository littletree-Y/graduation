...
def real_train_keywords(generator, discriminator, oracle_loader, config):
    ...
    oracle_keywords_file = os.path.join(sample_dir, 'oracle_keywords_{}.txt'.format(dataset))
    ...
    keywords_file = os.path.join(data_dir, '{}_keywords.txt'.format(dataset))
    test_file = os.path.join(data_dir, 'testdata/test_{}.txt'.format(dataset))
    ...
    test_loader = TestRealDataKeywordsLoader(batch_size, seq_len, keywords_len, repeat_times=num_sentences)
    ...
    x_keywords_onehot = tf.one_hot(x_keywords, vocab_size)  # batch_size x keywords_len x vocab_size
    x_fake_keywords_onehot = tf.one_hot(x_fake_keywords, vocab_size)  # batch_size x keywords_len x vocab_size
    assert x_real_onehot.get_shape().as_list() == [batch_size, seq_len, vocab_size]
    assert x_keywords_onehot.get_shape().as_list() == [batch_size, keywords_len, vocab_size]
    ...
    with init_sess() as sess:
        ...
        for _ in progress:
            ...
            feed = {x_real: batch, x_keywords:keywords, x_keywords_len_list:keywords_len_list,
                x_fake_keywords: fake_keywords, x_fake_keywords_len_list: fake_keywords_len_list}
            g_loss_np, d_loss_np, loss_summary_str, adv_keyword_loss_np  = sess.run([g_loss, d_loss, loss_summary_op, adv_keyword_loss], feed_dict=feed)
            sum_writer.add_summary(loss_summary_str, niter)

            sess.run(global_step_op)

            progress.set_description('g_loss: %4.4f, d_loss: %4.4f, adv_keywords_loss：%4.4f' % (g_loss_np, d_loss_np, adv_keyword_loss_np))
